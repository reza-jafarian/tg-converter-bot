...
async def init(bot):
    @bot.on(events.NewMessage(pattern='^(Check session|Check spam|Check T-data|Session to json|Session to txt|Session to string|T-data to session|Session to T-data|Enable 2FA|Reset 2FA|Disable 2FA)$', func=lambda e: e.is_private))
    ...
    @bot.on(events.NewMessage(pattern='^(• Mix zip files \(accounts\)|• Extract zip files \(accounts\))$', func=lambda e: e.is_private))
