...
def make_zip_file(zip_filename: str, file_path: str = None, file_name: str = None, file_content: dict = None, include_json: bool = True, folder_name: str = None) -> bool:
    try:
        temp_file = None
        
        if folder_name:
            if not os.path.exists(folder_name) or not os.path.isdir(folder_name):
                logger.error(f"Folder '{folder_name}' does not exist or is not a directory.")
                return False
        ...
        if file_name and file_content:
            temp_file = os.path.join(os.path.dirname(zip_filename), file_name)
            with open(temp_file, 'w', encoding='utf-8') as f:
                json.dump(file_content, f, indent=4, default=lambda obj: obj.isoformat() if isinstance(obj, datetime) else TypeError("Type not serializable"))
        
        mode = 'w' if not os.path.exists(zip_filename) else 'a'
        with zipfile.ZipFile(zip_filename, mode, zipfile.ZIP_DEFLATED) as zipf:
            if file_path:
                zipf.write(file_path, os.path.basename(file_path))
                json_file = file_path.replace('.session', '.json')
                if include_json and os.path.exists(json_file):
        ...
        logger.error(f'Make zip file error: {error}')
        return False

def make_txt_file(txt_filename: str, text: str) -> bool:
    try:
        ...
        logger.error(f'[-] Make txt file error: {error}')
        return False

def get_random_api(platform: str = 'desktop') -> dict:
    if platform == 'desktop':
        return {'app_id': 2040, 'app_hash': 'b18441a1ff607e10a989891a5462e627'}
    elif platform == 'android':
        return {'app_id': 4, 'app_hash': ''}
    else:
        return {'app_id': 0, 'app_hash': ''}

def get_random_app_version(platform: str = 'desktop') -> str:
    if platform == 'desktop':
        return random.choice(['5.1.7 x64'])    
    elif platform == 'android':
        return random.choice(['11.6.2', '10.14.5 (49452)', '10.14.5 (4945)'])
...
def get_random_device_model(platform: str = 'desktop', is_official_app: bool = False) -> str:
    if platform == 'desktop':
        device_models = [
            "0000000000",
            "0133D9",
...
def get_defualt_config(platform: str = 'desktop') -> dict:
    if platform == 'desktop':
        return {
            'app_id': 2040,
            'app_hash': 'b18441a1ff607e10a989891a5462e627',
            'device_model': 'ThinkPad T430',
            'app_version': '5.1.7 x64',
            'sdk_version': 'Windows 11 x64',
            'platform': 'desktop',
...
def generate_random_json(number: str, platform: str = 'desktop') -> dict:
    base_data = {
        'session_file': f'{number}.session',
        'phone': number,
        'device_model': get_random_device_model(platform = platform),
        'app_version': get_random_app_version(platform = platform),
        'sdk_version': get_random_system_version(platform = platform),
        'platform': platform,
...
def get_country_name(phone_number: str) -> Union[str, bool]:
    try:
...
async def tdata_to_session(path: str, number: str) -> str:
    try:
        tdata_folder = Path(path) / number / 'tdata'
        if not os.path.exists(tdata_folder):
            return 'unsuccess'
        
        tdesk = TDesktop(tdata_folder)
        assert tdesk.isLoaded()
        await tdesk.ToTelethon(session=f'{path}/{number}.session', flag=UseCurrentSession)
        
        return 'success'
    except Exception as error:
        print(f'[-][tdata_to_session] Error {path}: {error}')
        return 'unsuccess'
    ...
    async def process_account(account):
        ...
        if step in steps_with_telegram:
            telegram = Telegram(
                ...
                app_version      = json_data.get('app_version', json_data.get('version_app', '5.1.7 x64')),
                ...
                lang_code        = json_data.get('lang_code', (await get_country_language(country=get_country_name(number))).split('-')[0]),
                system_lang_code = json_data.get('system_lang_code', await get_country_language(country=get_country_name(number))),
        ...
        if step == 'check_session':
            ...
            response = make_txt_file(
                ...
                text = number if '+' in number else '+' + number
            )
            process_result['success' if response else 'unsuccess'] += 1
        
        elif step == 'session_to_string':
            string_session = await telegram.session_to_string()
            response = make_txt_file(
                txt_filename = f'({step})_Valid-{random_uniqe_code}.txt',
                text = string_session
            )
            ...
            await handle_response(await tdata_to_session(session_path, number))
